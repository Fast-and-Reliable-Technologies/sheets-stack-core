...
describe("Basic Database", () => {
  const dbP = DefaultBasicDatabase.instance();
  test("can fetch headers", async () => {
    const db: BasicDatabase = await dbP;
  ...
  test("can get row", async () => {
    const db: BasicDatabase = await dbP;
    const _row = 3;
    const actual = await db.getById(spreadsheetId, sheetName, _row);
  ...
  test("can search records with filter", async () => {
    const db: BasicDatabase = await dbP;
    const options: BasicDatabaseSearchOptions = { filter: { id: 2 } };
  ...
  test("can search records with single sort", async () => {
    const db: BasicDatabase = await dbP;
    const options: BasicDatabaseSearchOptions = { sort: "id", sortDesc: true };
    const actual = await db.search(spreadsheetId, sheetName, options);
  ...
  test("can search records with custom sort", async () => {
    const db: BasicDatabase = await dbP;
    const options: BasicDatabaseSearchOptions = {
      sort: ["isAdmin", "name"],
      sortDesc: true,
    };
    const actual = await db.search(spreadsheetId, sheetName, options);
